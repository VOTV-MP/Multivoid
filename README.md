...
|--|--|--|--|
| **Violet** (Discord) | A friend joining her session ran the game at ~9 FPS on Linux | The log triage exposed three real defects, all fixed at the class level: after any quit-to-menu the mod kept feeding a dead world's actors into engine calls (~2,500 absorbed access violations per second, flooding the log); every client world load broadcast ~870 spurious destroy events; and a once-per-second stutter everyone felt — thirteen subsystems each re-scanning the engine's whole object array — is now one shared, budgeted scan (frame cost capped at ~2 ms where spikes reached 300 ms). | b134 |
