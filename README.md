...
|--|--|--|--|
| **Violet** (Discord) | A friend joining her session ran the game at ~9 FPS on Linux | The log triage exposed five real defects, all fixed at the class level: after any quit-to-menu the mod kept feeding a dead world's actors into engine calls (~2,500 absorbed access violations per second, flooding the log); every client world load broadcast ~1,600 spurious destroy events (two separate causes — one silenced at its source, one because the quiet period ended before the world had finished rebuilding); a once-per-second stutter everyone felt — thirteen subsystems each re-scanning the engine's whole object array — is now one shared, budgeted scan; and the last periodic freeze, a full object-array census that could stall a frame for nearly two seconds on her machine, now runs spread over many frames (capped at ~1 ms each). Her friend's remaining low frame rate was measured NOT to be the mod: our whole per-frame cost is under a millisecond. | b134 |
