...
// button_start (NEW GAME), and opens the ImGui server browser (ui::server_browser)
// when it is clicked. Gameplay layer (principle 7): it owns the FEATURE -- which
// menu, where, what the click does -- and reaches the engine only through ue_wrap
// (InjectCanvasButton, WidgetIsHovered, and reflection for ui_menu_C's fields).
//
...
// save_button_disable, the isPause main-versus-pause discriminator included. A
// bind is reachable and simply not needed: this poll reads a real UButton and
