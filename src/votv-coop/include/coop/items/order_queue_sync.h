...
// every client (OrderQueue). Gameplay/network layer (principle 7): it reaches the
...
// client's drone never delivers (drone_sync), so a client's queue shrinks only by the host's pops.
