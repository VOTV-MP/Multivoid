// coop/creatures/kerfur_prop_adoption.h -- deferred class and pose adoption for PROP-form kerfurs.
//
...
// a DUPLICATE is spawned beside it, the twin left an untracked local kerfur once it loads.
//
...
// a last resort. The bound twin is then one host-range MIRROR: claimed and sweep-safe.
// CLIENT-only and GAME-THREAD-only, no mutex; the client-mint gate keeps the twin out of the
// Registry's actor-to-eid map, so after adoption it is purely a mirror. The prop-form analogue of
// coop/creatures/npc_adoption.h.
