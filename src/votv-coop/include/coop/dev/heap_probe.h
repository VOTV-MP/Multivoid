...
// the STATIC CRT (/MT), so those functions are baked into the payload DLL (main.dll) and no
...
// build/votv-coop/Release/main.map (the first non-CRT RVA in a chain is the
// owning call site).
//
// Discriminator:
