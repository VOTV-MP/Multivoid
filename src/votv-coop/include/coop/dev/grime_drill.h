...
//                 that resist the rain and whose process is above 20: one save, one set, and no fall of the
//                 weather's own on either), the client the first and the host the second, and run the same
//                 legs on their own one, in the rain's call form, clean(nullptr, Sub, noSound) through
//                 ProcessEvent: PARTIAL (Sub 10) and, once this peer's copy of the other peer's decal has
//                 fallen, DESTROY (Sub 1000, the process below zero). Each ends when its copy of the other's
//                 decal reads zero and is still alive -- an apply repaints, it never destroys. A phase that
//                 does not land in 15 s fails.
