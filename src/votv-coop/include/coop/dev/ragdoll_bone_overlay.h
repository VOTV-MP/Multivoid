...
//
// Two targets per update: the local player's own native ragdoll (mainPlayer.ragdollActor), drawn
// from the ragdoll's own head-camera view, and every remote peer's mirror body
// (RemotePlayer::RagdollBody); each is lit while that body is ragdolling. The mirror body's only coupled bone is the pelvis, and extending that is the point.
//
