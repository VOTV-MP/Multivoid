...
//             among those the lane names that gate a door and have a code of digits, and runs five
//             legs there, each started in one tick and ended by its own copy's state. PRESS the door
//             while the keypad is unlocked, first when it starts so (a joiner's door, no keypad
//             input): its open flips. ACCEPT the code on the numpad: active 1, the buffer empty, the
//             door's active 1. CANCEL two numpad digits, once they show, with "-": active 0. DENY a
//             wrong code on the keys and the accept key: active 0, the door's 0. TAIL, last, the same
//             and two digits more in that tick, which the host holds through the deny's 0.2 s tail:
//             the buffer ends as those two. Straight after each start its own copy reads as before.
