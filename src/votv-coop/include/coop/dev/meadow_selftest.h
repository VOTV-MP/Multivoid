...
// removes it; then a race, a row on each peer that the other has not seen yet.
...
//             out once X has left. Its DONE line says the lane sent exactly its verbs' lines and at least
//             one canonical order after the client's lines.
...
//             database to take the host's order, Y before X, which only the host's canonical brings. Its
//             DONE line says it sent exactly its own four lines, so nothing it applied went back out.
