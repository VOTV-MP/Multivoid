...
// session's end has nothing to restore. Which classes are refused, and which are left running
// because their anchor is the LOCAL player, is the table in docs/npcs-and-kerfur.md, "Which spawners
// a client refuses".
...
// Register the table's watches, once for the process (a name watch is never retired), and store
// `session` for the refusal's gate. Game thread (net_pump InstallObservers ensure).
//
