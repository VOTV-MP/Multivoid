...
// cadence; saved_signals::RowKey), and a row's cross-peer identity is signal_wire::ContentHash over
// its serialized, image-free blob. A 1 Hz positional diff runs under the append-at-tail invariant
// -- saveSignal and copySignal append, deleteSignal removes. An APPEND broadcasts the serialized
// row (SavedSignalAppend, host-relayed) and receivers replay the native gamemode.saveSignal, which
// rebuilds the pane, the specials and the forceObjects bookkeeping. A SHRINK broadcasts the removed
// hashes (SavedSignalDelete) and receivers resolve their own index by hash, then call deleteSignal;
// that covers the plain delete and the list side of the export-to-drive MOVE, while the drive's
// CONTENT is a known gap. A wire apply registers a key-to-hash mark the next poll adopts as sent,
// so an apply never echoes, and tombstones close delete-beats-append. World-down drops the shadow;
// it re-primes silently at world-up.
