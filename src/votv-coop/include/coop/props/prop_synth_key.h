...
// is None. The three non-Aprop_C classes here do NOT auto-mint -- a probe reads clump.GetKey as
...
// counter, so two peers minting concurrently cannot collide on identity lookup.
...
// already-indexed Key, which the game's own save data produces -- it ships clone families sharing
// one GUID, and the identity layer assumes uniqueness. Format `rk_<64-bit-random-hex>`, 19
// characters, inside the 31-character wire key field; random rather than the cs_ counter format, so
// a key PERSISTED into a save can never collide with a future boot's mints.
//
// Resolves setKey on the actual class first, since chip, clump and trashBits each declare their
// own, and falls back to the Aprop_C base for prop descendants -- FindFunction matches the exact
// owner and does not climb the superstruct chain. Returns the CONFIRMED re-read key on success, or
// L"" on any failure, where the caller keeps the old key and logs. Game thread only.
