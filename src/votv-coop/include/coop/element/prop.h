...
// container) has a Prop Element shadow whose ElementId is the handle the rest of the mod
...
// `prop_element_tracker::MarkPropElement`, from the Init POST observer or the seed scan, and
// Take'n back by `UnmarkKnownKeyedProp` on the K2_DestroyActor seam; the mirror one is
// Install'd by `remote_prop::RegisterPropMirror`.
