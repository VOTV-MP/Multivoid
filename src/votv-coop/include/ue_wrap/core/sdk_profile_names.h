...
inline constexpr const wchar_t* UiMenuButtonStartProp = L"button_start";      // UButton*  @ +0x2E0 (NEW GAME). The inject derives its parent VerticalBox from this button's slot AND memcpy-clones the BUTTON's FButtonStyle + tints + VBox slot layout (engine_widget.cpp:452-482) -- so ours looks/sounds native. What is NOT cloned is the LABEL: cloning tex_btnStart's UTextBlock style was TRIED and reverted (null at some inject timings -> Roboto/centered/white); engine_widget.cpp:394-424 sets the measured constants instead.
