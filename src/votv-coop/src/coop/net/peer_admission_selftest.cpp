...
//
// WHAT THESE ARMS ARE FOR, kept beside them because it is the reason they exist: a verifier that
// accepts everything passes every POSITIVE test there is. The happy path is proven by every smoke
// run and by the auth drill's control arm; only the negatives can show the gate is a gate. They are
// also the arms no LAN drill can stage -- a signature over a different direction byte, a different
// protocol version, a third party's key -- because staging one would mean building a peer that lies
// in a specific way.
//
// UN-GATED on purpose: it runs on every launch, not behind a dev flag, because a selftest that only
// runs when someone remembers to arm it is a selftest nobody has run. It has no production caller;
// it runs once at boot beside peer_identity's and lobby_password's.
...
bool RunSelftest() {
    ...
    // Two synthetic COUNTERPARTY identities. They are drawn as raw bytes rather than derived from a
    // keypair, and that is correct rather than lazy: the blob embeds 32 identity bytes as they are
    // and never interprets them, so what the negatives need is two values that DIFFER. Signing uses
    // the module's own real key, which supplies the asymmetry the negatives test.
    //
    // An earlier cut DID declare a keypair and then forget to derive the public halves, leaving pkA
    // and pkB both all-zero, so the third-party arm compared two byte-identical blobs and failed on
    // both peers, reporting a product defect that did not exist. Two inputs to a difference test
    // being accidentally equal is the failure mode a negative arm is most prone to, so they are
    // asserted distinct below rather than assumed.
    PubKey pkA{}, pkB{};
    ...
    Blob asClient;
    // THE TWO NONCE SLOTS GET DIFFERENT VALUES, so the layout assert below can actually see one of
    // them go missing. Passing `nonce1` twice made a BuildBlob that stopped writing the VERIFIER
    // nonce indistinguishable, at the byte level, from a correct one.
    ...
    // NEGATIVE 5 -- THE FLAGS ARE SIGNED. This is the arm that fails if the challenge's flag byte
    // ever leaves the blob again. Unsigned, a relay could set "password required" on an OPEN
    // lobby's challenge, and a bound client would derive and emit a real tag for a host that never
    // asked for one, turning an impersonation residual into a tag-harvesting oracle.
    Blob otherFlags;
