...
// run once per session start: no LAN run reorders a datagram, so the latch's refusals are proved on
// pinned sequences here, the reordered batch behind a taken one above all.
