...
//
// SRC-TREE PRIVATE (not under include/): only coingun_sync.cpp, coingun_arbiter.cpp and
// coingun_collect.cpp include it. The established pattern in this tree --
// coop/creatures/npc_sync_internal.h, ue_wrap/actors/puppet_internal.h,
// coop/props/remote_prop_internal.h -- for an extraction that splits one concept's file without
// splitting the concept itself: `coop/items/` still owns "the coin gun and its coins" whole.
//
...
//
// The collect lane needs this because such a coin is NOT a mirror and NOT map-placed -- it is a
// third case the non-mirror branch used to treat as level content, so a client overlapping its own
// pre-barrier coins credited itself. The answer at overlap time must be "suppress": if the shot
// goes on to author a sale the host mints the real coins and a local credit is a phantom; if it
// does not, the coin is RELEASED rather than destroyed and stays pickable, so suppressing costs a
// step off and back on, never the coin. Cheap: the held set is one shot's worth of coins for at
// most one pump interval, and empty otherwise. Any thread (takes the barrier lock).
...
// the reach check, the gun cache, sellObject/sell). It is driven from the sale lane's
...
// gun). The resolved UClass / UFunction / CDO pointers are not world-scoped and stay.
