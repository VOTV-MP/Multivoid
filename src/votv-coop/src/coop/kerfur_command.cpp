...
#include "coop/npc_sync.h"          // GetNpcIdForActor (host eid for the host's own menu use)
#include "coop/players_registry.h"  // Local() / Puppet(slot) / kPeerIdHost (the follow target body)
#include "coop/remote_player.h"     // RemotePlayer::GetActor (the puppet body actor)
