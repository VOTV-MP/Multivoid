...
void RunConfigSelftest() {
    ...
    if (!dirA.empty()) {
        ...
        if (::CopyFileW(src.c_str(), wrk.c_str(), FALSE)) {
            ...
            // Drill 1: ci-targeting + normalize. NOTE the key here is a CORPUS FIXTURE STRING, not
            // a live registry row -- `player_guid` is retired, and survives only as a note for an
            // ini that still carries the line. What is under test is the ini WRITE primitive's
            // case-insensitive first-occurrence targeting, which takes a raw key and knows nothing
            // about the registry; the fixture file supplies both spellings and that is all this
            // needs. inject.ini has Player_Guid=... ABOVE player_guid=...; writing 'player_guid'
            // must edit the FIRST (case-variant) line in place with canonical spelling -- count
