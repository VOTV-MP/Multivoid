...
// logged fallbacks (wallunit reelBig 0x0288 / reelSmall 0x028C).
...
void*   g_reelBaseCls = nullptr;  // Aprop_reel_C -- the slot lane's class test
...
// Failure backoff: EnsureResolved is called from the reel-slot poll and the caddy receivers. A
...
bool EnsureResolved() {
    ...
    g_offReelBig = big; g_offReelSmall = small;
    g_updFn = updFn;
    g_resolved = true;
    UE_LOGI("tape_caddy: resolved (reelBig=0x%X reelSmall=0x%X upd=%p)",
            g_offReelBig, g_offReelSmall, g_updFn);
    return true;
}
