...
| `coop/interactables/` | keyed device state | doors, lights, light groups, containers, garage, appliances, lockers: `interactable_sync` + `interactable_channel`; `keypad_sync` + `keypad_verbs`, `verb_lanes` (the lanes on a device's own verbs at the script gate, wired in one place), `power_sync`, `window_sync`, `grime_sync`, `turbine_sync`, `drone_sync`, `device_occupancy`, `garbage_sync`. The signal workstation: `signal_sync`, `signal_catch_sync`, `signal_wire`, `dish_sync`, `desk_input_sync`, `desk_sim_sync`, `desk_cursor_sync`, `desk_snd_fx`, `deck_play_sync`, `comp_sync`, `console_state_sync`, `drive_sync`, `drive_rack_sync`, `tape_caddy_sync`, `physmods_sync`, `serverbox_sync`, `laptop_sync`, `laptop_buffer_sync`, `floppybox_sync`, `floppy_slot_sync` and `floppy_slot_entry` (a device's disc slot: the state, and which overlap into it counts as an insert), `meadow_db_sync`. The ATV: `atv_sync`, `atv_corrector`, `atv_hit_guard`, `atv_condition_sync` |
