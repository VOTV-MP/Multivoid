...
| **`lib_C::addPoints(int32 Add, Object __WorldContext)` — EVERY credit/debit in the game** | `EX_Context(Default__lib_C)` → `EX_LocalVirtualFunction` at **all 19 call sites** | **INVISIBLE to BOTH PE and a Func patch** — the class above, and its most consequential instance | **Nothing today.** The one choke point is `lib_C::addPoints` and it needs the `0x45` GNatives substrate (`docs/COOP_VM_DISPATCH_PLAN.md`). The only alternative measured is Func-patching the NATIVE `VictoryBPFunctionLibrary::VictoryIntPlusEquals` it calls — reachable, but called from everywhere and very noisy. **Note what is NOT the answer: `AmainGamemode_C::AddPoints` is a 3-statement FORWARDER to this, not the writer** — `ue_wrap/world/economy.h` claimed otherwise for months and the false claim is why a census of OUR callers (one dev button) read like a census of the game's earnings. **This is what blocks host-authoritative economy** — not design; security A13/A34/A35 and the economy block in `docs/security/TRACKER.md` | [V bytecode census 2026-08-24 via `kismet-analyzer` over the extracted cooked tree; coverage checked (2,641 CXXHeaderDump classes vs 2,993 extracted `.uasset`); `main/lib.uasset`, `main/mainGamemode.uasset`] |
