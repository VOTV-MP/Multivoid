...
| **T1** | **REGRESSED / UNRELIABLE as of 2026-08-29 -- re-open before trusting this row.** It shipped 2026-08-26 (`95d18cc5`) as *the X and BACK exist and a driven click proves the X closes the screen* (`CLOSE BUTTON PASS`, hovered=1, real log). On 2026-08-29 the test FAILS reproducibly on PINNED bytes in three configurations, and the failure is NOT explained: `[V]` the button's `GetDesiredSize` is **(53,48)**, so it has a real hit area; `[V]` the SCRIM answers `IsHovered` in the very same runs, so the primitive works on our widgets there; `[V]` a 70-point sweep of the top-right 130x84 px never gets a hover, including the exact client point **(1394,246)** where one run DID find and successfully click it. Three causes were proposed and each falsified by measurement: `23481e3c`'s title-row rewrite; a boot MODAL of ours taking the mouse (retired since, and the failure outlived it); and a mounted `DebugMod.pak` (parked, still fails). A 1-tick move/sample settle was found and widened to 4 -- still fails; the shipped scrim phases use EIGHT, which is the next thing to try. THE ONE PASS came from an UNPINNED run, i.e. bytes another session had built, which is why it is not evidence. Instrumented meanwhile: the test now SEARCHES for the button instead of moving to a hard-coded estimate, and logs its geometry. Real `UButton`s cloning `ui_saveSlots_C.button_back`, so they carry the game's press/hover sounds; the clone has ONE owner now (`umg::CloneButtonStyle`, extracted from `InjectCanvasButton` under RULE 2). BACK sits at the footer's LEFT -- it was first built bottom-RIGHT, which is the CONFIRM position in every native window. **`Close()`'s cross-thread test is NOT done**: ESC and the X both call the internal `Hide()`, so the public path still has no caller and no proof. | T0 |
