...
| 4 | A UE4SS-lane build actually released | **NOT DONE** (re-verified 2026-08-28) — commit 3 LANDED (`1912d229`, §6 item 4 done) and the pipeline is inverted (C3.3); the remaining gate is only §6 item 5, the release itself. A zip has been ASSEMBLED locally and imported into r2modman successfully (2026-08-29: routing verified -- `mod/` landed in `shimloader\mod\Pelmentor-Multivoid\`, `pak/` in `shimloader\pak\Pelmentor-Multivoid\`, and the mod booted; that is precondition 6's control, passed for LAYOUT). **Do not reach for a named local file:** the build number moves several times a day (0.9.145 was superseded by 0.9.146 within the hour), and one of this session's own zips was assembled from a build directory a parallel session had just rewritten. Re-run `tools/release/package.ps1` and take the name it prints. |
...
| 6 | The negative control: our own zip imported locally into r2modman and the profile tree diffed against §7.2a's prediction | **DONE 2026-08-29 for LAYOUT** — the user imported `Pelmentor-Multivoid-0.9.145.zip` through Settings → Import local mod and the tree matched the prediction exactly: `mod/` → `shimloader\mod\Pelmentor-Multivoid\` (r2modman FLATTENS it, so `main.dll` sits at that folder's root, not under `dlls\`), `pak/` → `shimloader\pak\Pelmentor-Multivoid\` with `scientists.pak` + the four tiles, and the mod booted (`boot: Multivoid 0.9.0n b145`, HEALTH PASS, hooks installed). What this does NOT cover: the session itself never came up in that run — see `memory/project-r2modman-field-test-open-defects-2026-08-29.md`; the cause of the visible half is the master's build gate, not the package. **A SECOND IMPORT IS OWED AND THE USER HAS CLAIMED IT (2026-08-31): "я ещё должен буду локально нашу r2modman zip снова тестить".** It is now worth far more than the first, and **every obstacle in its way has been removed**: the gate that stopped the session (`COOP_MAX_BUILD`) is RETIRED (`24418b66`), and the master + relay were **cut over on 2026-08-31** on the user's instruction precisely so this test would not have to route around a stale server (`sig_gate` PASS 14/14 in production, `hostIdentity` on DIRECT `/v1/join`). So it needs no staged pair, no `ufw allow`, no custom endpoints — just the normal production ones. It is the first import that can reach a session, and therefore the first that can answer field defects #2 (tilde shows "No players") and #3 (F1 skin does not apply) before they are shipped **immutably**. Sequencing: `docs/RELEASE.md`, the flag-day section's "Before the day". |
