...
| chipPile/garbageClump CLIENT MIRROR | our own `SpawnActor`/`DestroyActor` (VISIBLE by construction — NO BP dispatch to observe) | **host-auth `AStaticMeshActor` PROXY (`coop/trash_proxy`, Movable, NoCollision):** `OnSpawn` trash-class branch → `SpawnProxy`; re-skin = `OnConvert`→`ReskinProxy`; retire = `OnDestroy`/disconnect→`RetireProxy`. Dup-gone + visible **[V]** hands-on (`245148c6`, deployed `8bc797ef`, HEAD `70d28df4`); the live carry-driven re-skin/follow is **[?] NOT working** — the held clump (in `holding_actor`) re-piles on cluster contact ~1/s (gate `@2927` checks `grabbing_actor`, null) → CHURN → each convert teleports the proxy (0.5–2 fps); option 1 FAILED, option 2 (the `holdPlayer` convert/ctx gate) pending. See `votv-chippile-carry-churn-holdplayer-gate-2026-06-22.md` | **host eid in `g_proxies` + the Prop mirror** (NO blueprint, `AddToRoot` → never self-morphs/GCs/stale) | trash_proxy |
...
> `8bc797ef`). The durable facts below (KEYLESS, the bytecode mechanic, the OBSERVABILITY) describe the
...
> but the **live clump carry is [?] NOT working** — the earlier "carry MIRRORS on a settled join / it was the
> JOIN RACE" claim is **WITHDRAWN as FALSE.** A real E-press carries the clump in `holding_actor`, so the
> native re-pile gate (`@2927`, checks `grabbing_actor` = null) never aborts → the held clump re-piles on
> cluster contact ~1/s → the game auto-re-grabs → CHURN → each convert teleports the client proxy (0.5–2 fps).
> Host carries fine; other props mirror fine; ONLY the chipPile is broken client-side. Option 1 FAILED; option
> 2 (the `holdPlayer` convert/ctx gate) is designed-not-built. See the proxy bullet under "The design (08…)"
> below + the new canonical finding
...
- **CLIENT MIRROR of trash = a host-authoritative `AStaticMeshActor` PROXY — DUP-FIX + VISIBILITY [V] hands-on
  VERIFIED; the LIVE CLUMP CARRY is [?] NOT working (the contact-re-pile churn). Deployed `8bc797ef` (HEAD
  `70d28df4`):** the client's mirror of a chipPile/clump is NO LONGER the real self-morphing BP — it is an
  ...
  **Movable** (`SetComponentMobility`, `245148c6`) because a runtime `AStaticMeshActor` defaults to STATIC, on
  ...
  mirror are **hands-on confirmed [V]**. **The LIVE CARRY does NOT mirror** — the earlier "carry MIRRORS on a
  settled join / the failure was the JOIN RACE" claim is **WITHDRAWN as FALSE.** A real **E-press** carries the
  clump in **`holding_actor`**, so the native re-pile gate (`@2927` checks `holdPlayer.grabbing_actor`, null
  here) **never aborts** → the held clump **RE-PILES on cluster contact ~1/s** (`StaticMesh.OnComponentHit`) →
  the game auto-re-grabs → **CHURN**; each `OnHostConvert` teleports the client proxy (`ClearAnyDriveFor` +
  `SetActorLocation`) → the client's clump morphs run **0.5–2 fps** + a stale pile lingers. **Host carries
  FINE** (native); **OTHER props mirror fine** (pose-stream); **ONLY the chipPile** (the lone convert-machine
  mirror) is broken client-side — stock VOTV behaviour (the client just shouldn't mirror it). The two clean
  smokes LIED because `autotest_chippile.cpp` grabs via `playerGrabbed` → the clump rides `grabbing_actor` →
  the gate aborts → no churn (the wrong, non-representative grab slot); render-blind + wrong slot = the false
  "proven." So the km-walk lerp/freeze is NOT meaningfully exercised by a real carry (the churn defeats it).
  **Option 1** (`8bc797ef`, `SetNotifyRigidBodyCollision(false)` on the held clump) BUILT + FAILED — the live
  host BP re-arms hit-notify. **Option 2 (DESIGN LOCKED, NOT BUILT):** gate the convert broadcast AND the ctx
  bump in `OnHostConvert` by the clump's **`holdPlayer`** (`@0x0240 AmainPlayer_C*`, CXXHeaderDump-confirmed) —
  `clump→pile` suppressed while `holdPlayer` non-null, `pile→clump` suppressed while a per-eid carrying flag is
  set; keep the local rebind → removes the CLIENT lag (the HOST churn stays, transient + self-cleaning).
  **Phase 1 = visual + position + re-skin, EXPLICIT NoCollision**; collision (the `garbageCollider` hull) is
  PHASE 2 with Increment 2. Design + AS-BUILT:
  ...
  **[V dup-fix + visibility; carry [?] NOT working — contact-re-pile churn, option 2 pending.]**
...
| client real-BP trash mirror self-morphs/GCs/goes-stale → `OnConvert` NOT-FOUND → spawn-fresh dup | **RETIRED** — the client mirror of trash is no longer a real BP; it is the host-authoritative `AStaticMeshActor` proxy (`coop/trash_proxy`, deployed `8bc797ef`): NO blueprint + `AddToRoot` + eid→actor registry + re-skin-in-place → the stale-mirror → spawn-fresh dup path is structurally unreachable. (Discriminator/health-poll/serial-check DROPPED as moot.) **Dup-gone + piles mirror visibly = hands-on confirmed.** *(Separate item, not a dup: the live clump CARRY is NOT working — the held clump (in `holding_actor`) re-piles on cluster contact ~1/s while the native gate checks `grabbing_actor` → CHURN → each convert teleports the proxy (0.5–2 fps); option 2 (the `holdPlayer` convert/ctx gate) pending; see the trash CLIENT MIRROR row above + `votv-chippile-carry-churn-holdplayer-gate-2026-06-22.md`.)* | [V — dup-fix hands-on] |
