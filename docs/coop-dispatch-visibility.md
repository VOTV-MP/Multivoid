...
| a portable PC's lid (`prop_portablePc_C::open`, the one writer of `opened`: the PC's actions 10 and 11 run it, so does its top's action 11, and the save does not write it) | Blueprint calls from the PC's own graph and its top's (a player's lid action) | no, to the detour | the script-body gate watches it on its own class BY NAME: the entry reads the lid, the exit sends the change (`coop/interactables/portable_pc_lid`, LaptopState op 6), and the host sends a client's line to every client, its author too; a receiver runs the same `open` inside the lane's scope, which sends nothing. `[V]` for the PC's own actions (the lid drill: the host's action-10 open and the client's action-11 close, each lane sending 1 line and applying 1); the top's action `[RD]` |
...
| a door's swing end (`move__FinishedFunc`, the move timeline's finish, which sets `isOpened` from the swing's direction) | the timeline's native tick | no, to the detour | the script-body gate watches it BY NAME, after the body only: on the host the door lane sends the state the swing settled at when the verb's intent did not already (a swing that settled otherwise, or intent fields that did not resolve); a client's copy ends the swings the lane applies, so nothing refuses it `[RD]`. The timeline's final tick stops it, updates it and only then calls this function, and a read between verbs can land in that window, the pump draining at an outermost dispatch: the door's intent reads a swing stopped at the end its dir points to, isMoving still set, as arrived (`TryReadFinishWindow` in `ue_wrap/devices/door`); a probe before each final update read 9 swings' windows `[V]` |
...
| a door's sensor events (`BndEvt__door_sensor_K2Node_ComponentBoundEvent_2_...` the begin, `..._3_...` the end, which build the list the autoclose counts; the names carry `K2Node_`, which the pseudo-C++ drops) | the sensor's overlap delegate, from the engine | yes: the stubs by name, 44 calls on the drill's door over a door drill run, 22 begins and 22 ends (`soak_625e294c_door`) [corr 2026-09-26: was "the stubs, no: 0 calls", measured by watches on the pseudo-C++ names, which match no function (`soak_32f310fc_door`: 0)]; the handlers too, as `ExecuteUbergraph_door` at the stubs' entry points (7956 the begin, 8242 the end), with the event's arguments in the ubergraph's frame: 27 of each on the same door, the five more each a second entry for a `coll` (an end) or a `door_R` (a begin) of a `door_C` (the log names the class, not which door) with no stub call beside it, not yet explained | read there by the door drill, a probe; no lane stands on them. The door's own `coll`, `door_L` and `door_R` begin and end on its sensor as the door moves; the begin handler adds only a prop or a pawn, so they never enter the list. `[V]` on a client |
...
| the Kerfus's player-0 reads, which name whom it follows: `GetPlayerPawn(0)` in `targetActor` (the path's goal, its no-task branch) and four `GetPlayerCharacter(0)` (its approach, stop and jump) | `EX_CallMath` into a native | not to the detour; yes to the native post-hook (the Func patch), whose source is the calling Kerfus | the host answers each with the puppet of the client that turned it on, from the record `coop/creatures/kerfus_follow` keeps in `coop/creatures/served_player`; the Blueprint's own branches do the rest, a task's goal and the possessed branch's null included `[V]` (`kerfus_drill`: a Kerfus a client turned on settles 1 m from that client and 11 m from the host, and the host's possess takes it 7 m away) |
| the Omega's player-0 reads, which name whom it serves: eight `GetPlayerPawn(0)` (its follow MoveTo and `targetActor`, its patrol's centre, its hand-back, its turn toward the player; murder mode's reach; its hum's pitch) and nine `lib.getMainPlayer()` calls (the disc insert's four, murder mode's three, the petting's, the uncalled `loadHoldItem`'s) | `GetPlayerPawn`: `EX_CallMath` into a native. `getMainPlayer`: `EX_Context` on `Default__lib_C` into a script function, whose body calls `GetPlayerCharacter` from the library's frame | `GetPlayerPawn`: not to the detour, yes to the native post-hook, whose source is the calling Omega. `getMainPlayer`: yes to the script-body gate by class name on `lib_C`, whose caller frame names the Omega and the variable the call writes; a post-hook on its inner `GetPlayerCharacter` sees the library's default object as the source | the host answers the served ones with the puppet of the client whose command the Omega runs (`served_player`, recorded by `coop/creatures/kerfur_command`): every `GetPlayerPawn` read, and of `getMainPlayer` only the insert's, told by the variable it writes, `CallFunc_getMainPlayer_AsMain_Player_1`. The sleep haunting's two `GetPlayerCameraManager(0)` reads and its wake-up punch through `gamemode.mainPlayer`, and the post-keys re-hold's `gamemode.mainPlayer`, pass no seam and stay the host's `[V]` (`kerfur_serve_drill`: a client's follow settles the Omega 1 m from that client, 15 m from the host, in the follow state, where without the `GetPlayerPawn` seam it settles by the host; its `get_reports` leaves the host's held disc in the host's hand) |
| the Omega's murder mode: `startKill`, the kill verb's event, which calls `move()` before it sets `kill` | a Blueprint self-call from `actionName` | not to the detour; yes to the script-body gate, by class name | its entry ends a client's service, so the chase's first `move()` reads player 0 (`kerfur_command`) `[V]` (`kerfur_serve_drill`: as the kill verb returns the Omega went for the host, where without the watch it went for the client it served) |
