...
| the Kerfus's player-0 reads, which name whom it follows: `targetActor` (the path's goal) and four `GetPlayerCharacter(0)` reads (its approach, stop and jump) | `targetActor` a Blueprint self-call from the ubergraph and the path functions; `GetPlayerCharacter` `EX_CallMath` into a native | `targetActor` yes to the script-body gate, by class name; `GetPlayerCharacter` not to the detour, yes to the native post-hook (the Func patch), whose source is the calling Kerfus | the host answers both with the puppet of the client that turned it on (`coop/creatures/kerfus_follow`) `[V]` (`kerfus_drill`: a Kerfus a client turned on settles 1 m from that client and 11 m from the host, where on main it settles by the host) |
