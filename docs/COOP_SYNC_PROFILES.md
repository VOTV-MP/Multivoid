...
| Balance | a CLIENT's legitimate earning | **BROKEN** | code | none | **No path exists.** `balance_sync.cpp:48-58` is host-poll + broadcast-on-change; the client only writes the host's absolute value. When a client's own game credits locally (coin pickup, drone sale, task reward) the host never learns and the next host-side change stomps it, so the earning is LOST and the peers disagree until then. Security **A13**. NOTE: the sentence previously in the row above claimed credits reach the balance host-side via "sells" — that was written 2026-08-24 as an assumption, never measured, and is false: there are zero `sell`/`coin` hits in the tree | n/a |
