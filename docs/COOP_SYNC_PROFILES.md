...
| ATV | rig pose + velocity (occupant OR host-idle) | U | code | CA/HA | `coop::atv_corrector::ApplyCorrection` (NOT `atv_sync` -- extracted 2026-08-30 `f802104e`; `SetTarget`/`LerpWindow` RETIRED 2026-08-29, the mirror simulates and is corrected, it is not interpolated). **Since `18edd22a` the LINEAR velocity is not written at all when the AUTHOR is linearly at rest** -- assigning one wakes a settled constraint rig and it sinks 25-40 cm (ATV.md 16); angular is written regardless, through `engine::SetActorRootPhysicsAngularVelocity` | snapshot adopt=1 (pose+velocity) |
