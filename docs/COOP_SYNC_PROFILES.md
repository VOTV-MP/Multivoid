...
the contents CRDT profiled here — profiled as its own row in §7's device-state table. The container
object thus spans TWO systems (contents + openable-device); this §1 covers only contents. That a
converged 8-facet worked example still had a whole sibling lane outside its frame is the §0 lesson in
...
**Update after the FULL-CATALOG sweep (§6–§7, ~67 systems, ~215 facets): ONE new VALUE, still NO new
...
- ✅ **`ContainerState`** — a SECOND container lane (open/closed device state, `interactable_sync` family),
  entirely separate from the contents CRDT §1 profiled. Now a row in §7's device-state table + a §1 note.
