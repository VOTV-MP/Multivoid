...
(3) **постеры**: `poster_c` = КАСТОМНЫЙ постер игрока (его картинки в паке нет — отсюда
пустышка `00000000texture`), класс в сейве = `poster.poster_c` БЕЗ `_C` → семья
`inst_poster_0..8` seeded (ноль пустышек в блендах); (4) z-fight: базовый лифт 13→20 мм +
пер-декальный джиттер 0–8 мм (`decal_lift`; стопки декалей больше не делят глубину), альфа-буст
×1.3 у grunge откачен (его причина — вывернутые нормали — убита в v7d); (5) все LIGHT-объекты
