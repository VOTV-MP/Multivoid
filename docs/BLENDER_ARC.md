...
**v16 (2026-08-30) — параметр Scale при импорте.** Юзер: «в аддоне отсутствует скейл параметр
при импорте» (а «учитывать unit scale из .blend» сам же отозвал — правильно: scale_length в
Blender дисплейная настройка, множить на неё геометрию = классический двойной скейл).
Корневой дизайн: `convert.SCALE` остаётся ЕДИНСТВЕННЫМ владельцем «Blender-единиц на UE-см»
(BASE_SCALE=0.01), `convert.set_scale(factor)` ставится ОДИН раз в начале `build_scene`;
оператор получил FloatProperty `Scale` (1.0 = реальный размер, 100 = 1 BU на UU; min 0.001);
smoke принимает `VOTVIO_SMOKE_SCALE`. Всё мировое, что было зашито МЕТРОВЫМИ литералами,
переведено в UU x convert.SCALE: константы проекции декалей (size_matrix 0.01x3, лифт 2uu +
джиттер 0.1uu, ячейка 12uu, допуски 2/25uu, планарный резидуал 1.5uu, span-floor 25uu),
тайлинг ландшафта (size_uu x SCALE) и triplanar Object-маппинг (/factor), радиус импорта
(семантика = метры ИГРОВОГО мира: radius_bl = m x 100 x SCALE в обоих гейтах — assemble.within
и landscape), empty-плейсхолдеры/камера (+65uu глаз, clip x factor). **Свет: ваттные
point/spot x factor^2** (обратный квадрат — сцена x f сохраняет освещённость); **эмиссия
материалов НЕ трогается намеренно** — у Blender'а strength поверхностей ПО ПЛОЩАДИ (W/m^2):
площадь x f^2 даёт ту же светимость поверхности и ту же освещённость соседей автоматически,
компенсация вручную = двойная. Приёмка: бенч радиус-150 на scale=1 ПОБАЙТНО равен v15
(1209/406/979/0 warnings), scale=2 даёт ИДЕНТИЧНЫЙ отчёт (инвариантность отбора и декалей)
при bbox ровно x2, light energy x4, cam_z/empty x2. Задеплоено юзеру (670 файлов).
