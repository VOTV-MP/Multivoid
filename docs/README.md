...
without modifying a single original game file. These pages describe what the mod is, what it
syncs, how it works, and how to work on it. Pick a lane; nobody reads this tree top to bottom. The
maintainer's working notes live outside the repository; what is here is the current state of the
mod and the rules for changing it.
...
One page per subsystem, all on the same skeleton: purpose, how it works, who owns what, the wire
messages, the late-join answer, the known limits with their evidence, and the code map.
