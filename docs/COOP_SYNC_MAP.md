...
| `world_actor_sync.cpp`, `host_spawn_watcher.cpp` | Non-Character event actors (B3b). 2026-07-04 late: +HostEnrollExSpawn (EX_CallMath spawns via npc_world_enum's source-gated Func-thunk — piramidSpawner_C proven PE-invisible) + pose-walk DEAD-RETIRE in TickPoseStream (event-end SELF K2_DestroyActor is PE-invisible; closed a latent mirror leak for all 17 WA classes). 2026-07-05 `ff338d87`: interceptor + ex-enroll + dead-retire HOSTING-gated (a pyramid spawned while the host was alone was never tracked -> the joiner's connect snapshot was empty -- the live 0s failure; sends stay peer-gated) | WorldActorSpawn/Destroy `[V; ex-enroll+dead-retire AS-BUILT, e2e 23:19; alone-spawn tracking fix code-verified, user re-test pending]` |
