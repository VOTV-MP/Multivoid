...
| container (`prop_container_C` family : prop_C + `propInventory_C` component; `prop_openContainer_C` = separate live-actor model) | [container.md](container.md) | RE DONE 2026-07-11 (GObjStack contents model + full verb census + dispatch visibility) + DESIGN (host-canonical contents-list state mirror, 1 Hz count-diff, key-addressed full-record payload; loot divergence closes by state overwrite — container.md §3). NOT BUILT |
