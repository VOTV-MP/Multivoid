...
- **A SETTER WITH NO CALLERS DISABLES EVERYTHING BEHIND ITS GATE -- and the design doc will still carry acceptance numbers for the branch that never ran.** `[V]` 2026-09-01: `ReseedGatePasses_` reads `g_reaperInGameplay`, whose only writer is `SetReaperInGameplayWorld` -- declared, defined and NEVER CALLED from `e1b1639f` (2026-08-23) until `40c0e086` (2026-09-01). `registry_reaper` computed the verdict and used it seven times in the same function without ever handing it over. So scan-hub consumer #14, which had REPLACED a retired 0.25 Hz census under RULE 2, collected ~3,200 candidates every two seconds and adjudicated none for nine days: `27 x "reason=gate"`, ZERO drains, on both peers. Its own DESIGN doc's AS-BUILT block reports "steady-state drain max = 1.004 ms" and "queues=4-6 bumps=4-6 /60 s" for that dark branch (now supersede-stamped). The damage surfaced two subsystems away -- the host key index held 28 keyed props against a world of ~2,200, so every coin-gun sale of a save-loaded prop was refused with the item already destroyed, and `SeedGeneration()` (the wake signal deferred joiners wait on) was frozen. **Look here FIRST:** when a subsystem's EFFECT is missing, grep its gate's writer for callers before reading its logic -- a hit list of "declaration, definition, two comments" is the whole answer for one command. Treat `collected N, dropped N` as an outage signal, not throttling, until you have counted the drains; and a RULE-2 replacement owes proof the new mechanism RUNS, because the old one is gone either way. [[lesson-a-setter-with-no-callers-disables-what-it-gates]]
