...
- **2026-08-24 — AN INSTRUMENT THAT PRIMES STATE THE REAL PATH DOES NOT PRIME PROVES ONLY ITSELF.** The v136 shop fix shipped with a GREEN and a RED transcript. `[V]` Its CLIENT half did not work at all: `store_catalog::NameOffset()` returned a CACHED offset and never BUILT the catalog, and `order_economy::ReadOrder` -- the first thing on the client's forward path -- asked only for that offset. Census of every `Ready()` caller: host-only `ResolveOne`, host-only `CommitOrder`, `RestoreCartItems` (client, but only AFTER a refusal, which needs a forward first), and **the dev drill**. So on a real client the catalog never built, every order silently failed to forward, and the client had ALREADY debited itself and disarmed its own delivery -- a money sink with no goods, strictly worse than the free-shop bug being fixed. The drill could not see it because it resolved its own rows through `Find()` -> `Ready()` **before** placing the order, warming the exact process-global production was missing. **This is not the blind-instrument failure -- the drill saw everything correctly; it CHANGED THE CONDITIONS so the thing under test could not fail.** It is easy to walk into because the setup looks like legitimate reuse (the catalog IS how you name a shop row), and it was *more convenient* in exactly the way that hid the defect. *Look FIRST:* enumerate the drill's setup line by line and mark each step -- **does the real trigger path perform this?** Anything unmarked is a contaminant, and **lazy init (`Ready()`/`Ensure()`/`Get()`) is the highest-risk shape**, because the drill advances it for free and you end up testing the second call. Prefer reaching the feature through the GAME'S OWN door (the rewritten drill runs `generateStore` + `addStoreCart` + `makeAnOrder` and touches none of our modules), and **print COLD in the transcript** so a reader can falsify the claim without re-deriving the call graph. Found by TWO independent post-ship audits converging -- the project's own strong signal, applied to a shipped commit rather than a stuck investigation. `memory/lesson_an_instrument_that_primes_state_the_real_path_does_not.md`

- **2026-08-24 — A BLOCKER STATED OVER A WHOLE DOMAIN IS USUALLY TRUE OF ONE *DIRECTION* ONLY, and its failure mode is CANCELLED WORK.** `[V]` All 19 of the game's credit sites are `EX_LocalVirtualFunction`, invisible to both the ProcessEvent detour and a `UFunction::Func` patch. True — and it was written up as *"host-authoritative economy is BLOCKED on the `0x45` substrate, not on design"*, in the auto-memory headline **and as step 2 of `COOP_SYNCER_MODEL.md` §2b's decision test**. The counter-example shipped the NEXT DAY: v136 charges a client's laptop purchase to the shared balance and never hooks `addPoints`, because a purchase leaves a **persistent, pollable artifact** (a row in `saveSlot.orders`) the acting client can point at, while a credit leaves only the number. So the DEBIT direction was never blocked. `[V]` Nor is all of the credit direction: `baocoin_C` is a world actor carrying an int `points` and its pickup is a component-bound multicast delegate (`ProcessDelegate -> ProcessEvent`), i.e. PE-VISIBLE — the sell gun has an artifact too. Only a credit whose sole trace IS the balance (point sack, chest, ATM, achievement) is genuinely blocked. *Look FIRST:* before writing "X is blocked on Y", **name the direction and try to falsify it in one sentence**; and split the observability question in two — *can the arbiter HOOK the trigger* vs *can the actor point at an ARTIFACT the arbiter can resolve* — because the second is usually available when the first is not, and it is strictly better anyway (an artifact-named intent carries an identity, not a value). A blocker that lands inside a **decision procedure** deserves a fix's scrutiny: it will be applied by someone with less context, and work that never gets attempted leaves no trace. `memory/lesson_a_blocker_stated_over_a_domain_is_usually_true_of_one_direction.md`

- **2026-08-24 — A STORED RECORD IS NOT THE SHAPE ITS CONSUMER SEES.** The whole v136 shop wire rests on `Fstruct_store.name` being the `list_store` row key, `[V]` stamped by `generateStore` @1419 — so the host was made to copy the LIVE table row **wholesale** into the committed order (retiring five version-coupled offset literals and fixing ~141 rows that mis-delivered because `asProp` was written as NAME_None). `[V]` But the table does not STORE that key: parsing the cooked export shows `name = "None"` on **all 473 rows** — the stamp goes into the SHOP SLOT's copy at runtime, not into the table. So a cart item built by the game's own `Button_order` and a row copied from the table are both `Fstruct_store` and are **not the same object**, and the wholesale copy stranded the one field the redesign had just made load-bearing. Caught only because a drill ran: the selftest's forward died with `ReadOrder(0) failed`. *Look FIRST:* before copying a record wholesale out of its store, **census what runs between the store and the consumer** (a generator, a spawn initialiser, a `PostLoad`, a UI adapter) — any of them may add a field the stored form lacks. **The tell is a field that is uniformly empty across every stored row**: 473/473 `"None"` is not "unused", it is "somebody else fills this in". And compare against what the ORIGINAL PRODUCER builds field by field, not against the schema — the question is never "is this a valid struct" but "is this the struct `Button_order` would have produced". `memory/lesson_a_stored_record_is_not_the_shape_its_consumer_sees.md`

- **2026-08-24 — AN ACCEPTANCE TEST NEEDS THE *CONSUMER'S* REQUIREMENT, not just a correctness check.** The STEP-0 probe scored three `list_store` readers against a frozen offline truth (473 rows, sum 73271, two digests — one over `name=price`, one over `name` alone, so a right-set/wrong-price failure stays distinguishable). It was decisive about one candidate (`GetDataTableRowFromName` dispatched all 473 rows, returned false every time, never wrote the out slot — unusable from C++) and **useless about the other two: both matched all four terms.** The criterion could not choose, and it was not neutral — the design doc already called the cleaner-looking one *"the cleanest winner if it works"*. `[V]` The deciding term is not a correctness term at all: the consumer memcpys the live ROW BYTES, and `GetDataTableColumnAsString` returns `TArray<FString>` and can never hand back a pointer at any level of correctness. The walk shipped; the runner-up became the **runtime gate**, a better job than the one it was about to be given (and it retired a worse gate — `sellObject`, ambiguous by construction for the 113 odd-priced rows). *Look FIRST:* write the criterion **from the consumer backwards** — pointer? owned buffer? lifetime? synchronous? cost bound? — before the "is it correct" terms. **If two candidates can pass every term, the criterion is incomplete**, and that is visible when you write the terms down, before the run. Ask what the runner-up is GOOD at before discarding it, and distrust any prose that pre-ranks the candidates. `memory/lesson_an_acceptance_test_needs_the_consumers_requirement.md`
