...
- **SEND-SIDE EVIDENCE IS NOT DELIVERY EVIDENCE -- and four green facts can all be about the wrong thing.** `[V]` 2026-09-01: I published a new sync lane as "live cross-peer" and two post-ship audits found it had never reached a receiver -- the new `ReliableKind` was missing from its family switch, so `event_feed` logged `unknown ReliableKind 129 -- dropping` per packet. Everything I had cited was real and none of it was delivery: *"connect-snapshot sent 42 states"* is the SENDER; the identical index `keysHash` is both peers' LOCAL scans; *"census: 0 disagreements"* was taken at idle, where there is nothing to disagree about; and the selftest called the apply function DIRECTLY, bypassing the wire. **Look here FIRST:** for any wire lane the acceptance line lives in the RECEIVER's log and names the sender (`<lane>: applied ... (from slot N)`) -- if you cannot name that line, the lane is untested; the negative control is one grep for `unknown ReliableKind <N>` on both peers; and a direct-call selftest proves the APPLY, never the ROUTE. This is the third shipment of that class after v70 `DeskLogLine` and v71 `SleepState`, so the memo checklist is not enough -- a machine gate over the enum vs the family dispatchers is the owed fix. [[lesson-send-side-evidence-is-not-delivery-evidence]]

- **A MUTATING DEV PROBE BREAKS THE RUN IT MEASURES -- and the obvious gate is the wrong predicate.** `[V]` 2026-09-01: arming `lightswitch_probe` on the host cost the client its join (`expected 2 peers, got 1` with the flag on, `PASS` with it off, same DLL, twice) because its one-shot world write lands inside the joining client's connect-snapshot. Two traps: that failure string is this project's known signature of a CONCURRENT SESSION killing your run, so the cheap explanation was a rig collision and it was wrong -- `_FRIENDLY_SESSION.txt` is what ruled it out BEFORE a re-run; and gating on `connected()` does not help, because a host that has not yet accepted a client is not "connected" and that IS the window. **Look here FIRST:** gate a writing probe on `running() || connected()`, keep read-only and mutating instruments on SEPARATE ini rows, and de-braid a two-peer failure by re-running with one flag flipped and nothing else. Cost to state honestly: this makes a mutating selftest unrunnable in every automated scenario until the harness gets a session-less launch. [[lesson-a-mutating-probe-breaks-the-run-it-measures]]
