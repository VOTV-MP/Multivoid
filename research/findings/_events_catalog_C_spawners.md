...
| `ticker_wispSpawner` | wisp variants — `wisps` `TMap<TSubclassOf<AActor>,float>` @0x0240 weighted table | `ReceiveTick` re-rolls interval; `skipFirst` bool | observable | anomaly prop | unhandled | MED |
| `ticker_yellowWispSpawner` | yellow wisp -> spawns `deer` (`deer` array @0x0260); uses `mats` PhysicalMaterials to pick surface; `P` FVector spawn pt | `ReceiveTick` | observable | anomaly + creature | unhandled | MED |
