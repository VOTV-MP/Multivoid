...
| **`updateVolumesAndMass` never re-derives on extraction** | OPEN -- measured: records 7->6 while `currVol` stayed 28579.0 on BOTH peers. **RECONCILED 2026-07-23: the fix is NOT "needed" -- it SHIPPED.** `container_contents_sync.cpp:448-475` `ResolveRederiveFns`/`RederiveManagedState` resolve the verb from the DECLARING class (`411743af`, 19:13), and the 07-22 v125 take (later, 21:08) STILL showed staleness known-open -- so either that smoke's DLL predated the fix or a second cause remains. Authority is `none` (convergent-local, each peer re-derives from the synced content -- NOT a host stream), see `docs/COOP_SYNC_PROFILES.md` §4 facet #6 (container; renumbered from §1 when the master table moved to the top) | gate:none -- **needs a fresh hands-on to confirm the shipped fix; not a fresh code fix** | co-requisite of R11b |
