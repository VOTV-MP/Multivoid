...
| **R-4a** | **FULLY BUILT 2026-08-23 late eve (both halves)** | Key-scoping half `65fccd70` (871 → 0). **End-condition half BUILT (9-round /qf "that holds")**: the RECONCILE WINDOW — a second flag beside the untouched episode, raised at Arm/reload-arm/SnapshotBegin, lowered at SnapshotComplete/flake-backstop/180 s ceiling; destroy seam suppresses any kind, drop-intent only kind==load (curtain-anchored classifier). Burst re-attributed by counting: ~870 clump-eidN = the twin-retire kernel (ALREADY cause-scope-fixed via MarkIncomingDestroy in `save_time_retire_util.h`) + ~660 KEYED eid=0 = untaggable engine/BP rebuild churn (the window's class). RED/GREEN drilled: 5 injected keyed destroys broadcast under the old edge, suppressed under the new (same keys). Design: `architecture-audits/votv-r4a-end-condition-DESIGN-2026-08-23.md`. |
