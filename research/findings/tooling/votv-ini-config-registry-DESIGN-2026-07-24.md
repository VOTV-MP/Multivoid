...
**Date:** 2026-07-24 (pass 3: 2026-07-25) · **Status:** **ARCS 1+2 BUILT 2026-07-25 (see §6 as-built
notes; arc 2 = commits `06e9910d..90b43f82`); arcs 3-4 DESIGN** · **Passes:** `/qf` pass 1 = 16
...
| **3** | **IN BUILD 2026-07-25** per the certified impl design `votv-ini-arc3-impl-DESIGN-2026-07-25.md` (/qf 16r "that holds"): C1 `f88a78cf` + C2 `faa0289d` + C3a `3b9aba38` BUILT (typed row defaults aliasing owning constants; handle-only read API, ResolveString; 62-site flag sweep; IsIniKeyTrue deleted -- READ ratchet closed). Remaining: C3b write door + enum_check retire, C4 T2b, C5 registry_gate CI + drills + AFTER-compare + smoke. BEFORE harvest: 100 keys / 108 sites / 0 conflicts. | in build |
