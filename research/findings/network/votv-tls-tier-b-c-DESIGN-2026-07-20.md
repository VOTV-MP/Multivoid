...
| F5 | Live box: **:80 free**, certbot **absent**, :443/:8443 = a foreign tenant's xray (no reverse proxy possible). Units are `DynamicUser=yes` + `ProtectSystem=strict`; secrets already ride `EnvironmentFile` (root-read, then privileges drop) | `ss -tlnp` on 172.86.94.3; `vps_provision.sh:94-120,175` |
