...
kProtocolVersion **122** (the s29 version-identity lane below bumped it — the Join
payload gained the game-target field; a 121-or-older peer HARD-CLOSEs at the gate —
RELAUNCH BOTH PEERS). THIRTEEN unverified layers v112..b122 + the s23 trio above;
prefixes attribute.

## What changed in b122 (2026-07-19 s29 — version identity, "multivoid") — proto 122

The mod's version identity is now the Paper pair (game + build): display
**"votv-coop 0.9.0n b122"** (top-left menu label, browser header, boot banner), the DLL
file is **multivoid-0.9.0n-122.dll** (the xinput proxy scans multivoid-*.dll, loads the
highest build; duplicates/legacy votv-coop.dll -> the in-game "MOD INSTALL PROBLEM"
popup). Browser Version column shows "0.9.0n b122" per row; amber "(!)" = the join WILL
be refused with a popup. Join gate = byte-equality on (game, build) at three layers
(browser pre-flight popup / Join-seam wire gate / header backstop). Update check is
informational-only (the master serves "no released record" until the first release).
**STEPS (b122, quick):** (a) top-left label reads "votv-coop 0.9.0n b122"; (b) browser
header + your own row show the same pair; (c) normal join works unchanged (the gate is
invisible between equal builds). The mismatch/dup branches are already drill-verified
with real logs + a popup screenshot (s29; votv-version-identity-v122-DESIGN doc) — no
hands-on needed unless you want to see the dup popup: drop any copy of an older
multivoid-*.dll beside the current one and boot.
