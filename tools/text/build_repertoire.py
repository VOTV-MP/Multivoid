...
def main():
    ...
    # --- 3c. THE CASE-FOLD TABLE --------------------------------------------
    #
    # Same generator, same repertoire, for the reason arc D2 built one generator:
    # a table describing what the fold set CONTAINS must be minted from the fold
    # set, not written by hand beside it. The hand-written one claimed to cover
    # "exactly the cased scripts the repertoire draws" and was 73% self-folding
    # by the time anyone counted. It lives in case_fold.py only because this file
    # is at its 800-line soft cap.
    case_ranges = case_fold.runs(rep)
    cased = case_fold.census(rep, case_ranges, unicodedata, sys.exit)
    print(f"casefold: {cased} cased-and-drawable codepoints in {len(case_ranges)} "
          f"runs (frozen Unicode {case_fold.UNICODE_VERSION}, live census clean)")
    ...
    case_fold.emit(case_ranges, GEN, lambda name: [h.format(name=name) for h in head])
