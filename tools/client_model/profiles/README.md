...
One json per learned example: a base of profiles, not one file.
...
- A manual-pose PSK next to the .mdl beats the library: the converter LEARNS the
  ...
  manual poses; future models with a similar skeleton auto-pick it.

| file | learned from | look | status |
|---|---|---|---|
| `tpose_v1_narrow_2026-07-01.json` | `hl_einstein_v1sc.psk` (22-bone HL Bip01) | narrow arm span 177.1 (the first manual example) | **active** — the in-game look preferred |
| `tpose_v2_wide_2026-07-02.json` | `hl_einstein_v1sc_new_profile.psk` | wide arm span 209.5, matches the anthro template proportions (215) | **rejected** — the in-game look was vetoed; auto-select skips it |
| `tpose_rvi38_2026-07-02.json` | `rvi_scientist_v1sc_my_pose_good.psk` (38-bone Sven Co-op Bip01: +toes/fingers/head-prop) | a manual rvi pose, span 194.8, height 188 | **active** |

**Format 3 (the only supported format):** per-bone
...
the height flips the argmax and the model gets measured/grounded sideways (a
sideways-grounded model with a large residual was the symptom).
